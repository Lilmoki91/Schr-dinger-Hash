...
def calculate_quantum_signature(user_id, seed_hex):
    ...
    probs = [abs(amp)**2 for amp in result.final_state_vector]
    state_obj = {"pairs": 8, "qubits": 8, "state_vector": [round(float(p),10) for p in probs]}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⚛️ *ANU QUANTUM ENTANGLEMENT GATE*\n\n📌 Klik *'LOG MASUK'*", reply_markup=reply_markup, parse_mode="Markdown")
...
async def handle_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"<b>[⚛️ ANU QUANTUM]</b>\n\nHash: <code>{quantum_hash[:16]}...</code>\nAkses: <code>{count}</code>", parse_mode="HTML")
