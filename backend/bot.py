...
def calculate_quantum_hash_entanglement(user_id, seed_hex):
    """Litar Entanglement 8-Qubit. Output: STATE VECTOR (deterministik)."""
...
# ============================================================
# ⚛️ HASH CALCULATION API ENDPOINT
# ============================================================
class QuantumHashHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        if self.path == '/api/calculate-hash':
            ...
            if not user_id or not seed_hex:
                ...
                self.end_headers()
                self.wfile.write(b'{"error": "Missing user_id or seed_hex"}')
                return
...
def start_hash_server():
    server = HTTPServer(('0.0.0.0', HASH_API_PORT), QuantumHashHandler)
    logging.info(f"🔑 Hash Calculation API running on port {HASH_API_PORT}")
...
def main() -> None:
    ...
    # Start Hash API server dalam thread berasingan
    hash_thread = threading.Thread(target=start_hash_server, daemon=True)
    hash_thread.start()
    ...
    print(f"🔑 Hash API: Port {HASH_API_PORT}")
    print("=" * 60)
