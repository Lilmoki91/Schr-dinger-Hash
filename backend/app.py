...
@app.route('/')
def home():
    return jsonify({"status": "Quantum Entanglement Gate Active", "version": "4.0"})
...
async def handle_contact(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if str(contact.user_id) != user_id:
        await update.message.reply_text("❌ Entropi fizikal tidak sepadan.")
        return
    ...
    try:
        ...
        token_data = {"u":user_id,"s":seed_hex,"h":quantum_hash,"exp":expiry,"tid":token_id,"qid":quantum_matrix_id,"src":hash_source}
        token_safe = urllib.parse.quote(base64.b64encode(json.dumps(token_data).encode()).decode())
        
        msg = (
            f"<b>[⚛️ ORIGIN QUANTUM]</b>\n\n"
            f"<b>🔗 Entanglement:</b> <code>8 pasangan CNOT</code>\n"
            f"<b>• Hash:</b> <code>{quantum_hash[:16]}...</code>\n"
            f"<b>• State:</b> <code>{q_state_visual}</code>\n"
            f"<b>• Akses ke-:</b> <code>{count}</code>\n"
            f"<b>• Token sah:</b> <code>{ttl_seconds}s</code>"
        )
